...
public class EntityDrawable implements VariableDrawable
{
	...
	public EntityDrawable(String imageLocation)
	{
	...
	{
		projectionMatrix.identity().ortho(0, scale.x, 0, scale.y, 0, 10);
		//projectionMatrix.ortho(0, scale.x, 0, scale.y, 0, 10);
		translationMatrix.identity().translate(x, y, 0);
		...
		program.setUniformMatrix4fv("projectionMatrix", projectionMatrix);
		program.setUniformMatrix4fv("translationMatrix", translationMatrix);
	...
	{
		render(offset.x, offset.y, scale);
	}
	...
	private void createProgram()
	{
		...
		vs.create();
		vs.loadSource(readSourceFile("/shaders/variable.vs"));
		vs.compile();
